...
#ifndef RS_MATH_H_INCLUDED // added
#define RS_MATH_H_INCLUDED // added
...
public:
    ...
    /// Addition operator
    Angle &operator+=(const Angle &b) // return *this is OK
    {
        *this = Add(*this, b);
        return *this;
    }
...
template <typename T, std::size_t N> // This is just Vector
class VectorBase
{
protected:
    T _elem[N];
    ...
    VectorBase(void) // fill the element of _elem[N] from _elem to _elem+N with T(0) 
    {
        std::fill(_elem, _elem + N, T(0)); // std::fill -> fill the element in the array with the value
    }
...
template <typename T>
inline Vector<T, 3> Cross(const Vector<T, 3> &a, const Vector<T, 3> &b) // Cross Product
{
    return Vector<T, 3>(
        a[1] * b[2] - a[2] * b[1],
        a[2] * b[0] - a[0] * b[2],
 ...
 */
template <typename T>
class Quaternion // Quanternion calculation
{
private:
    T _a, _x, _y, _z;
